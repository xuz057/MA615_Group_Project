...
##So I use the second plot to compare the difference between basketball and baseball. From this bar chart, we can find the average basketball attendance has much little difference under the different level of rains. So our group thinks that the influences of rain on the basketball attendances is less than baseball attendance.
```{r}
rainlevel<-Attendance_rain[,c("Date","PRCP")]
rain_level_c<-inner_join(rainlevel,table_celtics,by="Date")
Attendance_rain2<-rain_level_c%>%
  group_by(PRCP)%>%
  summarise(mean_attendance=mean(Attendance))
p_c2<-ggplot(Attendance_rain2, aes(PRCP, mean_attendance))+geom_bar(stat = "identity") 
p_c2
p_red2
